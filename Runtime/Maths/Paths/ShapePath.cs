...
namespace MaroonSeal.Maths.Geometry.Paths {
    abstract public class ShapePath : IShapePath, ISerializationCallbackReceiver
    {
        ...
        abstract public Transform3D GetPointAtTime(float _t);
        public Transform2D GetPoint2DAtTime(float _t) {
            Transform3D point = this.GetPointAtTime(_t);
        ...
        public Transform3D GetPointAtDistance(float _distance) {
            return GetPointAtTime(GetTimeAtDistance(_distance));
        }
        public Transform2D GetPoint2DAtDistance(float _distance) {
            Transform3D point = this.GetPointAtDistance(_distance);
            return new(point.position, point.rotation, point.scale) { Right = point.Forward };
        }
