...
namespace MaroonSeal.Maths.Geometry.Paths.ReedsShepp {
    ...
    /// </summary>
    [System.Serializable]
    public class ReedsSheppPath : CompositePath<PrimativePath>
    {
        ...
        [Space]
        private Transform2D startPoint;
        public Transform2D StartPoint => startPoint;
        ...
        public ReedsSheppPath(Transform2D? _start = null, float _minRadius = 1.0f, List<RSManoeuvre> _manoeuvreList = null) : this() {
            
            startPoint = _start ?? new Transform2D();
        ...
        #region Manoeuvres
        public void SetManoeuvres(List<RSManoeuvre> _manoeuvreList, Transform2D? _start = null, float? _minRadius = null) {
            Clear();
        ...
        private void AddManoeuvre(RSManoeuvre _manoeuvre, bool _refresh) {
            ...
            if (manoeuvreList.Count == 0) {
                ...
                anchorPoint = segments[^1].EvaluateTime(1.0f);
                gearChangeCount += _manoeuvre.gear != manoeuvreList[^1].gear ? 1 : 0;
            }

            PrimativePath manoeuvrePath = _manoeuvre.steer == RSManoeuvre.Steer.Straight ? 
        ...
        public RSManoeuvre GetManoeuvreAtTime(float _t)
        {
            ...
            int index = GetIndexAtTime(_t, out float _localTime);
            return manoeuvreList[index];
        }

        public RSManoeuvre GetManoeuvreAtDistance(float _distance) => 
            GetManoeuvreAtTime(DistanceToTime(_distance));
        #endregion

        #region Manoeuvre Paths
        private LinePath GetStraightManoeuvrePath(Transform2D _start, RSManoeuvre _manoeuvre) {
        ...
        private ArcPath GetTurnManoeuvrePath(Transform2D _start, RSManoeuvre _manoeuvre) {
            float theta = _manoeuvre.travel * (int)_manoeuvre.gear * Mathf.Rad2Deg;

            Transform2D arcTransform = new(_start.position, _start.angle - 90.0f, new(-(int)_manoeuvre.steer, 1));
        ...
        static public ReedsSheppPath FindShortestPath(Transform2D _start, Transform2D _end, float _minRadius = 1.0f, float _gearChangeWeight = 0.0f) {
        ...
        static public List<ReedsSheppPath> GetOrderedPathCases(Transform2D _start, Transform2D _end, float _minRadius = 1.0f, float _gearChangeWeight = 0.0f) {
            List<ReedsSheppPath> pathCases = GetPathCases(_start, _end, _minRadius);
        ...
        static public List<ReedsSheppPath> GetPathCases(Transform2D _start, Transform2D _end, float _minRadius = 1.0f) =>
